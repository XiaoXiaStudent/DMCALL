...
/***********设备操作函数;以下函数中的m_hdl是指Ap5isNXMcaWr返回的句柄*******/
//打开端口获取句柄;
//  Nbr是端口号，无论是双头模块还是单头模块，都是从1开始，依次为2/3/4...，最大126。电脑上插入一个就始终是1；插入n个 端口分别是1、2....n
//  使用Ap5isNXMcaWr接口打开默认vid pid的盒子
HANDLE WINAPI Ap5isNXMcaWr(int Nbr);
//使用i4dUMV3JPnm1接口可以打开指定vid pid的单头盒子或者双头盒子的主控端。必须保证电脑上只插有一个这种盒子。
...
/***********键盘操作函数;以下函数中的m_hdl是指Ap5isNXMcaWr返回的句柄*******/
/***********以下所有命令返回 0: 成功；-1: 失败*******/
...
/***********鼠标操作函数;以下函数中的m_hdl是指Ap5isNXMcaWr返回的句柄*******/
/***********以下所有命令返回 0: 成功；-1: 失败*******/
...
//返回当前鼠标坐标X值
int WINAPI uqi3e0KmrV(HANDLE m_hdl); //由于某些语言不支持指针，增加此接口
//返回当前鼠标坐标Y值
int WINAPI Q9g7UfQrNUK(HANDLE m_hdl); //由于某些语言不支持指针，增加此接口
